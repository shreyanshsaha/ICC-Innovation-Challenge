...
// Number of TIMER1 cycles between voltage reading checks (240 = approx 30
// seconds)
#define BYPASS_COUNTER_MAX 0

// Default i2c SLAVE address (used for auto provision of address)
#define DEFAULT_SLAVE_ADDR 24
...
// If we receive a cmdByte with BIT 6 set its a command byte so there is another
// byte waiting for us to process
...
volatile uint16_t VCCMillivolts = 4200;
...
// empty all the address data in the EEPROM to 0
void factory_default();
// Reboot function to reset the watchdog timer and disable timer 1
void Reboot();
// Just delay here so the buffers are all ready before we service i2c
void wait_for_buffer_ready();

/*
 * Panic mode incase of any issue
 */
void panic();

/*
 * Calculates checksum to verify EPROM data
 *
 * @param unsigned integer
 * @param unsigned integral data type
 */
...
/*
 * Averages analog values and multiply it with calibration value
...
void setup() {
  ...
  */
  ...
  // WDTCSR configuration:     WDIE = 1: Interrupt Enable     WDE = 1 :Reset
  // Enable Enter Watchdog Configuration mode:
  WDTCR |= (1 << WDCE) | (1 << WDE);

  // Set Watchdog settings - 4000ms timeout
  WDTCR = (1 << WDIE) | (1 << WDE) | (1 << WDP3) | (0 << WDP2) | (0 << WDP1) |
          (0 << WDP0);
...
void loop() {
  ...
  if (last_i2c_request > 0) {
    // Count down loop for requests to see if i2c bus hangs or controller stops
    // talking
    last_i2c_request--;
  }

  // If we are on the default SLAVE address then use different LED pattern to
  // indicate this
...
//    if (ByPassCounter >= 0) {
//      // We are in ACTIVE BYPASS mode - the RESISTOR will be ACTIVE + BURNING
//      // ENERGY
//      ByPassCounter--;
//      digitalWrite(PB4, HIGH);
//
...
void WriteConfigToEEPROM() {
  ...
  // EEPROM.put(EEPROM_CHECKSUM_ADDRESS, calculateCRC32((uint8_t*)&myConfig,
  // sizeof(cell_module_config)));
}
...
bool LoadConfigFromEEPROM() {
  ...
  // uint32_t checksum = calculateCRC32((uint8_t*)&restoredConfig,
  // sizeof(cell_module_config));
...
void LEDReset() { green_pattern = GREEN_LED_PATTERN_STANDARD; }

void factory_default() { EEPROM.put(EEPROM_CHECKSUM_ADDRESS, 0); }
...
void sendByte(uint8_t number) { Wire.write(number); }
    ...
    switch (cmdByte) {
      ...
      case COMMAND_set_bypass_voltage:
        if (howMany == sizeof(uint16_t)) {
          ...
          // Only accept if its a realistic value and the value is LESS than the
          // last voltage reading
          if (newValue >= MIN_BYPASS_VOLTAGE &&
              newValue <= MAX_BYPASS_VOLTAGE && newValue < VCCMillivolts) {
      ...
      case COMMAND_set_slave_address:
        ...
        if (howMany == 1) {
          ...
          if (newAddress != myConfig.SLAVE_ADDR &&
              newAddress >= DEFAULT_SLAVE_ADDR_START_RANGE &&
              newAddress <= DEFAULT_SLAVE_ADDR_END_RANGE) {
    ...
    switch (cmdByte) {
      case read_voltage:
        // VCCMillivolts = Update_VCCMillivolts();
          VCCMillivolts -= DECREASING_RATE;

          if (VCCMillivolts <= 3000) {
            VCCMillivolts = 4200;
          }
        flash_green = true;
        break;
...
float Update_VCCMillivolts() {
  // Oversampling and take average of ADC samples use an unsigned integer or the
  // bit shifting goes wonky
  uint32_t extraBits = 0;
  for (int k = 0; k < OVERSAMPLE_LOOP; k++) {
    extraBits = extraBits + analogVal[k];
  }
  // Shift the bits to match OVERSAMPLE_LOOP size (buffer size of 8=3 shifts,
  // 16=4 shifts)
...
// function that executes whenever data is requested by master (this answers
// requestFrom command)
void requestEvent() {
  switch (cmdByte) {
    // V=F
    ...
    case read_temperature:
      // sendUnsignedInt((uint16_t)((float)temperature_probe *
      // myConfig.TemperatureCalibration));
      sendUnsignedInt((uint16_t)13);
      break;
...
//      sendFloat(17.17);
  ...
  // Reset when we last processed a request, if this times out master has
  // stopped communicating with module
...
void initADC() {
  /* this function initialises the ADC
    ...
    --------------------
     ...
     ADC Prescaler needs to be set so that the ADC input frequency is between 50
    - 200kHz.
  ...
  */
  ...
  // NOTE The device requries a supply voltage of 3V+ in order to generate 2.56V
  // reference voltage.
  ...
  // Internal 2.56V Voltage Reference without external bypass capacitor,
  // disconnected from PB0 (AREF) ADLAR =0 and PB3 (B0011) for INPUT (A3) We have
  // to set the registers directly because the ATTINYCORE appears broken for
  // internal 2v56 register without bypass capacitor
  ADMUX = B10010011;
...
static inline void initTimer1(void) {
  ...
  OCR1C = 64;  // About eighth of a second trigger Timer1  (there are 488 counts
               // per second @ 8mhz)
