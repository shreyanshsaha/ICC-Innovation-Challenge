...
void avg_balance() {
  ...
  Serial.print("Average cell voltage in float is currently : ");
  Serial.println(avgint*1000);
  Serial.println();
  Serial.println();
}
...
void loop() {
  ...
  if (cell_array_max > 0) {  
    ...
    if ((millis() > next_submit) && (WiFi.status() == WL_CONNECTED)) {
      ...
      max_enabled=false;
      if (max_enabled!=true){
        myConfig.autobalance_enabled = true;
    ...
    Serial.println("The Slave board is inactive!");
  }
  delay(31000);
}//end of loop
